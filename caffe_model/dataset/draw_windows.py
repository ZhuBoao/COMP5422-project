...
def plotstft(audiopath, binsize=2**10, plotpath=None, colormap="gray", channel=0, name='tmp.png', alpha=1, offset=0):
    ...
    ims = ims[0:256, 0:2048] # 0-11khz, ~10s interval
...
file = open('./samples/samples.csv', 'r')
for iter, line in enumerate(file.readlines()[1:]): # first line of traininData.csv is header (only for trainingData.csv)
    filename = line.split(',')[0]
    # filename = filepath[:-4]
    wavfile = 'tmp.wav'
    command = 'mpg123 -q -m -n 1000 -w' + wavfile + ' "C:/Users/zbozb/OneDrive/life_in_polyu/semester_1/COMP5422_Multimedia Computing/Group_Project/Python/samples/' + filename+'"'
    print command
    os.system(command)
    """
    ...
    plotstft(wavfile, channel=0, name='C:/Users/zbozb/OneDrive/life_in_polyu/semester_1/COMP5422_Multimedia Computing/Group_Project/Python/samples/'+filename+'.png', alpha=1.0)
    os.remove(wavfile)
