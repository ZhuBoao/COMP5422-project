...
def plotstft(audiopath, binsize=2**10, plotpath=None, colormap="gray", channel=0, name='tmp.png', alpha=1, offset=0):
    ...
    # ims = ims[0:256, offset:offset+768] # 0-11khz, ~9s intertrain
...
file = open(thispath+ '/train/samples.csv', 'r')
labelStr = ''
os.system('mkdir '+thispath+'/spectrograms/')
os.system('mkdir '+thispath+'/spectrograms/train')
for iter, line in enumerate(file.readlines()[1:]):
    filename = line.split(',')[0]
    label = line.split(',')[1]
    wavfile = 'tmp.wav'
    for augmentIdx in range(0, 10):
        command = 'mpg123 -m -n500 -k'+str((augmentIdx+1)*500)+' -w' + wavfile + ' "'+thispath+'/train/' + filename+'"'
        print command
        os.system(command)
        plotstft(wavfile, channel=0, name=thispath+'/spectrograms/train/'+str(iter)+'_'+str(augmentIdx)+'.png',
                 alpha=1.0)
        labelStr = labelStr + str(iter) +'_'+str(augmentIdx)+'.png' + ' '+ str(label)
    os.remove(wavfile)
    print "processed %d files" % (iter + 1)
with open(thispath+"/spectrograms/train/data.txt", "w") as text_file:
